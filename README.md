...
formErrors: IOrderFormErrors — Объект для хранения ошибок валидации формы.
...
getOrderData(items: string[], total: number): IOrderBatch — Возвращает объект с данными заказа, включая способ оплаты, контактную информацию, адрес, общую стоимость и список товаров.
